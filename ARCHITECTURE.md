...
| File | Globals |
|------|---------|
| data/topics.js — 36 topic packs (vocabulary / situation / pattern), 393 pairs total. Pattern pairs carry a `slottable` array of eligible blank positions (pattern markers excluded); Pattern Practice picks one at random per round. A legacy `slot` field == `slottable[0]` is kept for backward compat. | TOPICS |
